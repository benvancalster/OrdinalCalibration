...
mlrlpx1 = log(mlrpred[, 1] / (1 - mlrpred[, 1]))
mlrlpx2 = log((mlrpred[, 1] + mlrpred[, 2]) / (1 - mlrpred[, 1] - mlrpred[, 2]))
mlrlpx3 = log((mlrpred[, 1] + mlrpred[, 2] + mlrpred[, 3]) / (1 - mlrpred[, 1] - mlrpred[, 2] - mlrpred[, 3]))
mlrlpx4 = log((mlrpred[, 1] + mlrpred[, 2] + mlrpred[, 3] + mlrpred[, 4]) / mlrpred[, 5])
# did not work:
mlrvgamsmps4x = vgam(cad$o5 ~ sm.ps(mlrlpx1,df=4) + sm.ps(c(mlrlpx2),df=4) + sm.ps(c(mlrlpx3),df=4) + sm.ps(c(mlrlpx4),df=4),family=cumulative(parallel=F))
# worked:
mlrvgamsmps4xx = vgam(cad$o5 ~ sm.ps(mlrlpx1,df=4) + sm.ps(c(mlrlpx2),df=4) + sm.ps(c(mlrlpx3),df=4) + sm.ps(c(mlrlpx4),df=4),family=multinomial(refLevel = "1"))

mlrlpy1 = log(mlrpred[, 1] / (1 - mlrpred[, 1]))
mlrlpy2 = log(mlrpred[, 2] / (1 - mlrpred[, 2]))
mlrlpy3 = log(mlrpred[, 3] / (1 - mlrpred[, 3]))
mlrlpy4 = log(mlrpred[, 4] / (1 - mlrpred[, 4]))
# did not work:
mlrvgamsmps4y = vgam(cad$o5 ~ sm.ps(mlrlpy1,df=4) + sm.ps(c(mlrlpy2),df=4) + sm.ps(c(mlrlpy3),df=4) + sm.ps(c(mlrlpy4),df=4),family=cumulative(parallel=F))
# did work:
mlrvgamsmps4yy = vgam(cad$o5 ~ sm.ps(mlrlpy1,df=4) + sm.ps(c(mlrlpy2),df=4) + sm.ps(c(mlrlpy3),df=4) + sm.ps(c(mlrlpy4),df=4),family=multinomial(refLevel = "1"))

# ECI
mlrECIr = eci_rel(calout=mlrvgamsmps4,preds=mlrpred,k=5,outc=cad$o5)
eci_rel(calout=mlrvgamsmps4xx,preds=mlrpred,k=5,outc=cad$o5)
eci_rel(calout=mlrvgamsmps4yy,preds=mlrpred,k=5,outc=cad$o5)

# Calibration scatter plots and curves per outcome category and per outcome dichotomy
pdf("CARDIIGAN_MLR_Calibrationplots_yy.pdf")
par(mfrow=c(2,2))
plotscatter5(preds=mlrpred,obs=mlrvgamsmps4yy,cexaxisv=1,cexlabv=1.2,cexlegendv=0.8)
plotlines5(preds=mlrpred,obs=mlrvgamsmps4yy,cexaxisv=1,cexlabv=1.2,cexlegendv=0.8)
oplotscatter5(preds=mlrpred,obs=mlrvgamsmps4yy,cexaxisv=1,cexlabv=1.2,cexlegendv=0.8)
oplotlines5(preds=mlrpred,obs=mlrvgamsmps4yy,cexaxisv=1,cexlabv=1.2,cexlegendv=0.8)
dev.off()
