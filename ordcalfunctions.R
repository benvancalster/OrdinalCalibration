...
plotscatter5 <- function(preds,obs,cexv=0.8,cexaxisv=1.4,cexlabv=1.5,cexlegendv=1.2,maintxt=NULL){
  plot(preds[,1],fitted(obs)[,1],type="p",pch=1,col="green",lwd=1,ylab="Observed proportion",xlab="Estimated probability",xlim=0:1,ylim=0:1,main=maintxt,cex=cexv,cex.axis=cexaxisv,cex.lab=cexlabv)
  points(preds[,2],fitted(obs)[,2],pch=1,col="orange",lwd=1,cex=cexv)
  points(preds[,3],fitted(obs)[,3],pch=1,col="red",lwd=1,cex=cexv)
  points(preds[,4],fitted(obs)[,4],pch=1,col="brown",lwd=1,cex=cexv)
...
flexrecalk5_cardiigan <- function(filenm, res){
  vgamsmps4azz = vgam(cad$o5 ~ sm.ps(log(res[,2]/res[,1]),df=4) + sm.ps(log(res[,3]/res[,1]),df=4) + 
                               sm.ps(log(res[,4]/res[,1]),df=4) + sm.ps(log(res[,5]/res[,1]),df=4),family=multinomial(refLevel = 1))
  vgamsmps4axx = vgam(cad$o5 ~ sm.ps(log(res[,1]/(res[,2] + res[,3] + res[,4] + res[,5])),df=4) + 
                        sm.ps(log((res[,1] + res[,2])/(res[,3] + res[,4] + res[,5])),df=4) + 
                        sm.ps(log((res[,1] + res[,2] + res[,3])/(res[,4] + res[,5])),df=4) + 
                        sm.ps(log((res[,1] + res[,2] + res[,3] + res[,4])/res[,5]),df=4),family=multinomial(refLevel = 1))
  vgamsmps4ayy = vgam(cad$o5 ~ sm.ps(log(res[,1]/(1 - res[,1])),df=4) + 
                        sm.ps(log(res[,2]/(1 - res[,2])),df=4) + 
                        sm.ps(log(res[,3]/(1 - res[,3])),df=4) +
                        sm.ps(log(res[,4]/(1 - res[,4])),df=4), family=multinomial(refLevel = 1))

  vgamsmps4azzz = vgam(cad$o5 ~ sm.ps(log(res[,2]/res[,1]),df=4) + sm.ps(log(res[,3]/res[,1]),df=4) + 
                                sm.ps(log(res[,4]/res[,1]),df=4) + sm.ps(log(res[,5]/res[,1]),df=4),family=cratio(parallel = F))
  vgamsmps4axxx = vgam(cad$o5 ~ sm.ps(log(res[,1]/(res[,2] + res[,3] + res[,4] + res[,5])),df=4) + 
                         sm.ps(log((res[,1] + res[,2])/(res[,3] + res[,4] + res[,5])),df=4) + 
                         sm.ps(log((res[,1] + res[,2] + res[,3])/(res[,4] + res[,5])),df=4) + 
                         sm.ps(log((res[,1] + res[,2] + res[,3] + res[,4])/res[,5]),df=4),family=cratio(parallel = F))
  vgamsmps4ayyy = vgam(cad$o5 ~ sm.ps(log(res[,1]/(1 - res[,1])),df=4) + 
                         sm.ps(log(res[,2]/(1 - res[,2])),df=4) + 
                         sm.ps(log(res[,3]/(1 - res[,3])),df=4) +
                         sm.ps(log(res[,4]/(1 - res[,4])),df=4), family=cratio(parallel = F))

  ecifrm = c(eci_rel(calout=vgamsmps4azz,preds=res,k=5,outc=cad$o5),
             eci_rel(calout=vgamsmps4azzz,preds=res,k=5,outc=cad$o5),
             eci_rel(calout=vgamsmps4axx,preds=res,k=5,outc=cad$o5),
             eci_rel(calout=vgamsmps4axxx,preds=res,k=5,outc=cad$o5),
             eci_rel(calout=vgamsmps4ayy,preds=res,k=5,outc=cad$o5),
             eci_rel(calout=vgamsmps4ayyy,preds=res,k=5,outc=cad$o5))
  
  save(res, vgamsmps4azz, vgamsmps4azzz, vgamsmps4axx, vgamsmps4axxx, vgamsmps4ayy, vgamsmps4ayyy,  ecifrm, file=filenm)
